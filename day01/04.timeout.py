"""
timeout 超时
1. 上传文件，上传2M耗时比较短，但是2G的文件，耗时比较久，可以使用tinmeout设置比较长的时间
...
"""
代理 proxies
1. 用界面操作某个功能，结果正常。但是用自动化操作同意的功能，报错。检查问题所在处
    界面操作时，抓包
    自动化脚本执行时，抓包
    对比抓到的包，检查差异点
2. 频繁的向服务器发起请求，服务器当作攻击处理，将IP地址禁掉了。使用代理IP发送请求
"""
url = "http://192.168.150.222:8081/futureloan/mvc/api/member/list"
proxy = {
...
r = requests.get(url, proxies=proxy)  # 设置代理后，要把对应的代理根据fiddle打开
print(r.text)
